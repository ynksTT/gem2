...
$(function() {
  ...
  $(".nav_img p").mouseover(function() {
    ...
    // 全てのコンテンツを非表示にする
    $("#review_img div").stop(true, true).fadeOut(400);
    ...
    } else if(tagId === 'img_art') {
      fadeTimeout = setTimeout(function() {
        $(".img_art").fadeIn(400);
      }, 300); // 前のフェードアウトが完了してから0.3秒後に同時にフェードイン
    } else if(tagId === 'img_furniture') {
      fadeTimeout = setTimeout(function() {
        $(".img_furniture").fadeIn(400);
      }, 300); // 前のフェードアウトが完了してから0.3秒後に同時にフェードイン
    }
  });
});
...
$(".readmore").on("click", function() {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    // 監視する要素を取得
    const targetElementAbout2 = document.querySelector('#review2');
